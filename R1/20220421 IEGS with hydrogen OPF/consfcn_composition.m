function g = consfcn_composition(PGs0,Qptg0, gasFlow, composition_hy,composition_gas, mpc, signGf, nGb,nGl)
%% convert
...
for i = 1:nGb
        ...
        for m = 1:nGl % get all the bus and Gline connected as injection
            if ((mpc.Gline(m,2) == i) && (signGf(m) == 1)) 
                fromGlineIndex = [fromGlineIndex,m];
                fb = [fb, mpc.Gline(m,1)];
            elseif ((mpc.Gline(m,1) == 1) && (signGf(m) == -1))
                fromGlineIndex = [fromGlineIndex, m];
                fb = [fb, mpc.Gline(m,2)];
            end
        end
