function om = GEopf_setup(mpc, mpopt)
...
%add
%初始变量有，节点气压初始值，气源供气初始值；
Prs = mpc.Gbus(:,4);%虽然不是控制变量，但是迭代初始时用到
PGs = mpc.Gsou(:,2);
LCg = zeros(nLCg,1); 

%约束变量有：（如果多了没事，可以设为无穷大）:气源出力上下限(气压上下限?这里不定义下面也得定义的)
Prsmin = mpc.Gbus(:,5);
Prsmax = mpc.Gbus(:,6);
PGsmin = mpc.Gsou(:,3);
PGsmax = mpc.Gsou(:,4);
LCgmin = zeros(nLCg,1);
LCgmax = mpc.Gbus(mpc.Gbus(:,3)~=0,3);
%----- test-----
% LCgmax=zeros(size(LCgmax));
LCgmax = LCgmax .*0.8;
%add
...
if dc
  ...
  %add
  %格式：om，变量名，个数，变量初始值，下限，上限
  om.add_var('Prs',nGb,Prs,Prsmin,Prsmax);
  om.add_var('PGs',nGs,PGs,PGsmin,PGsmax);
  om.add_var('LCg',nLCg,LCg,LCgmin,LCgmax);
  %add
  %% nonlinear constraints
  om.add_nln_constraint(mis_cons, [nb;nb], 1, fcn_mis, hess_mis, nodal_balance_vars); % active and reactive nodal power balance
  ...
  %add OM.add_nln_constraint(NAME, IDX_LIST, N, ISEQ, FCN, HESS, VARSETS);
gas_nodal_balance_vars = {'Va','Vm','Pg','Qg','Prs','PGs','LCg'};
gas_flow_lim_vars = {'Va','Vm','Pg','Qg','Prs','PGs','LCg'};
[vv, ~, ~, ~] = om.get_idx();
fcn_Gmis = @(x)opf_Gmis_fcn(x,vv,mpc);
hess_Gmis = @(x,lam)opf_Gmis_hess(x,lam,vv,mpc);
fcn_Gf = @(x)opf_Gf_fcn(x,vv,mpc);
hess_Gf = @(x,lam)opf_Gf_hess(x,lam,vv,mpc);
%
om.add_nln_constraint('Gmis', nGb,1,fcn_Gmis,hess_Gmis,gas_nodal_balance_vars);%节点气守恒
om.add_nln_constraint('Gf',nGl,0,fcn_Gf,hess_Gf,gas_flow_lim_vars);%线路天然气流量不越限
  %add
  %% linear constraints
