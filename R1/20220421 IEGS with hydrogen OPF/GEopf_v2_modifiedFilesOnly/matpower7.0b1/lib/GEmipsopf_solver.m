function [results, success, raw] = GEmipsopf_solver(om, mpopt)
...
%add 把气网这部分初值替换掉，以防气压相等的情况出现
x0(vv.i1.Prs:vv.iN.Prs) = mpc.Gbus(:,4);
x0(vv.i1.PGs:vv.iN.PGs) = mpc.Gsou(:,4);
%add
...
mpc.Gbus(:,7) = Prs;
mpc.Gsou(:,5) = PGs;
mpc.Gbus(find(mpc.Gbus(:,3)~=0),10)=LCg;
%add
...
%add calculate gas flow according to the optimization results of nodal gas
%pressure
...
%add question?怎么修正？
muGl= 2*Lambda.ineqnonlin((2*nl2+1):(2*nl2+nGl)); %% nl2: number of constrained lines
%add
...
%add
mpc.Gbus(:,8)=Lambda.upper(vv.i1.Prs:vv.iN.Prs);
mpc.Gbus(:,9)=Lambda.lower(vv.i1.Prs:vv.iN.Prs);
mpc.Gsou(:,7)=Lambda.upper(vv.i1.PGs:vv.iN.PGs);
mpc.Gsou(:,8)=Lambda.lower(vv.i1.PGs:vv.iN.PGs);
mpc.Gbus(:,11) = -Lambda.eqnonlin((nne.i1.Gmis:nne.iN.Gmis));%节点气价
%add
%% package up results
...
%add
% nl_mu_u=[nl_mu_u;zeros(nGb,1);muGl];
%add
