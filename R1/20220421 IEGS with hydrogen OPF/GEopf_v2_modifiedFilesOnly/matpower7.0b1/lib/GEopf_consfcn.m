function [h, g, dh, dg] = GEopf_consfcn(x, om, Ybus, Yf, Yt, mpopt, il, varargin)
...
%% add rows of g,... and modify the original g rows
% [vv, ~, ~, ~] = om.get_idx();
% mpc = om.get_mpc();
% g = add_g(g,x,vv,mpc);
% h = add_h(h,x,vv,mpc);
% dg =add_dg(dg,x,vv,mpc);
% dh =add_dh(dh,x,vv,mpc);
